...
public class IosSimulatorInstallAppTask extends DefaultTask {
    ...
    public void doTaskAction() throws IOException {
        ...
        if (execResult.isSuccess()) {
            ...
            throw new IOException("Failed to install app on iosSimulator");
        }
    }
    ...
    private IosApp getAppInfo(File ipa) throws IOException {
        File payloadDir = new File("build/gmp-temp/"+ipa.getName());
        ZipUtil.extractAll(ipa, payloadDir);
        IosApp iosApp = new IosApp(CommonUtil.findIosAppsInDirectory(payloadDir).get(0));
